	...
	if (shortcuts.GetCount()) {
		...
		for (int i = 0; i < shortcuts.GetCount(); i++) {
			...
			if (mainDlg->shortcutsBottom) {
				...
				button->Create(shortcut.label, WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_CHECKBOX | BS_PUSHLIKE, buttonRect, this, IDC_SHORTCUT_RANGE + i);
				...
				if (shortcuts.GetCount() > 12) {
					int row = i % 2;
					button->Create(Translate(shortcut.label.GetBuffer()), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_CHECKBOX | BS_PUSHLIKE, CRect(shortcutsRect.right - 198 + row * 97, shortcutsRect.top, shortcutsRect.right - 4 - (1 - row) * 97, shortcutsRect.top + buttonHeight), this, IDC_SHORTCUT_RANGE + i);
					...
					button->Create(Translate(shortcut.label.GetBuffer()), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_CHECKBOX | BS_PUSHLIKE, CRect(shortcutsRect.right - 138, shortcutsRect.top, shortcutsRect.right - 4, shortcutsRect.top + buttonHeight), this, IDC_SHORTCUT_RANGE + i);
...
BOOL Dialer::OnInitDialog()
{
	...
	m_ButtonVoicemailDisabled.LoadBitmaps(IDB_VMAIL_GREY, IDB_VMAIL_GREY_DOWN, IDB_VMAIL_GREY_FOCUS);
